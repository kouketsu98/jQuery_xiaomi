...
(function() {
	...
	$.ajax({
		...
		success: function(res) {
			...
			// $('h4.xm').text(res.data.receiveName);
			// $('span.ph').text(res.data.receivePhone);
			// $('p.address-ph').text(res.data.receiveRegion + res.data.receiveDetail);
			console.log(res.data.id);
			
			// console.log($(`.dialog-address-info[data-id = ${res.data.id}]`).find('.dialog-address-checkbox'));
			// var actives = $(`.dialog-address-info[data-id = ${res.data.id}]`).find('.dialog-address-checkbox');
			...
			$.ajax({
				...
				success: function(res) {
					if(res.code !== 200) {layer.msg(res.msg); return;}
						...
						layer.msg('设置默认地址成功');
						// console.log($(`.dialog-address-info[data-id = ${id}]`).children('.dialog-address-checkbox'));
						// $(`.dialog-address-info[data-id = ${id}]`).children('.dialog-address-checkbox').addClass('.active');
						return;
					}
				},
			});
...
// "ids": 购物记录id购成的整型数组(不要传空数组),"account": 要生成订单的总金额,"addressId": 要生成订单的送货地址的id
	...
	// 	// window.location.href =  `/order/order.html?ids=${ids}`;
	// 	console.log(idss);
